...
interface DeleteTreatmentModalProps {
  ...
  onCloseModal: () => void;
  onSubmmit: (treatmentId: number) => void;
  isSubmitting: boolean;
}

export function DeleteTreatmentModal({
  showModal,
  treatment,
  onCloseModal,
  onSubmmit,
  isSubmitting,
}: DeleteTreatmentModalProps) {
  return (
    <Modal dismissible show={showModal} onClose={onCloseModal}>
      <Modal.Header>Deletar tratamento</Modal.Header>
      <Modal.Body>
        <div className="text-center">
          <HiOutlineExclamationCircle className="mx-auto mb-4 h-14 w-14 text-red-500 dark:text-gray-200" />
          <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
            {`Você tem certeza que deseja deletar o tratamento "${treatment.description}?"`}
          </h3>
          <div className="flex justify-end gap-4">
            <Button
              color="red"
              isLoading={isSubmitting}
              onClick={() => onSubmmit(treatment.id)}
            >
              Deletar
            </Button>
